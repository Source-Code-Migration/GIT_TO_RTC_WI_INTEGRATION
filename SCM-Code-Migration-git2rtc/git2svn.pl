...
do {
    ...
    open GIT, 'GIT_DIR=git/.git git log --all --format=format:%H,%P,%at,%an|'
        or die "git log: $!";
    for my $line (<GIT>) {
        chomp $line;
...
system('svn', 'checkout', "https://laptop-88ur2qnf/svn/test22", 'svn') == 0
    or die "svn checkout: $?";

svn('mkdir', 'trunk');
svn('mkdir', 'branches');
svn('mkdir', 'tags');
#svn('commit', '--username', 'admin', '--password', 'admin', '-m', 'mkdir {trunk,branches,tags}');
...
sub commit {
    ...
    svn( '--username', 'admin','--password','admin', '--username', $commit->{author}, 'commit', '-m', $commit->{message});
    ...
    for my $tag (@{$commit->{tags}}) {
        ...
        svn('commit', '--username', 'admin','--password','admin', '-m', "tag $tag");
        svn('update');
    }
